...
def inverse(list_of_blocks):
    ...
    """
    for block in list_of_blocks:
        block = cv.idct(block/1.0)
        block = fix_inverse(block)
...
def recomp(list_of_blocks, original_shape,F):
    ...
    #        for y in range(0, F):
    #            m_x = ( ( (i % n_block_x) ) * (F) ) + x 
    #            m_y = int( np.floor( i / n_block_x) * F) + y
    #            matrix[m_x][m_y] = list_of_blocks[i][x][y]
    #print(matrix)
    block_row = []
...
F = 300
d = 600
JPEG_compession(path+ '\\40682662_ml.jpg', F, d)
